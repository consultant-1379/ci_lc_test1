...
String projectsFile = readFileFromWorkspace('ci-jenkins-config/projects_acceptance_unit')
projectsFile.eachLine {
    project_name -> createPipelinePreCodeAcceptanceJob(project_name)
}

def createPipelinePreCodeAcceptanceJob(project_name) {
    ...
    pipelineJob('Pipeline_Acceptance_Unit_' + project_name.split('/').last()) {
        ...
        triggers {
            ...
            definition {
                cpsScm {
                    ...
                    scriptPath('ci-jenkins-config/jenkinsFiles/ciPipelineJenkinsFiles/ciPipelinePreCodeAcceptanceJenkinsFile.groovy')
                }
            }
        }
    }
}
listView('Pipeline_Acceptance_Unit') {
    filterBuildQueue()
    filterExecutors()
    jobs {
        regex(/Pipeline_Acceptance_Unit.*/)
    }
    columns {
        status()
        weather()
