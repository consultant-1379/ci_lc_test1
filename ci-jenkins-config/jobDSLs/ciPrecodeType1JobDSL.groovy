...
String projectsFile = readFileFromWorkspace('ci-jenkins-config/projects_precode_review_type_1')
...
def createPipelineJob(project_name) {
    ...
    pipelineJob(project_name.split('/').last()+'_Precode_With_SonarQube') {
        ...
        triggers {
            ...
            definition {
                cpsScm {
                    ...
                    scriptPath('ci-jenkins-config/jenkinsFiles/ciPipelineJenkinsFiles/ciPrecodeReviewType1.groovy')
                }
            }
        }
    }
}
listView('Precode With Sonarqube QG') {
    filterBuildQueue()
    filterExecutors()
    jobs {
        regex(/.*_Precode_With_SonarQube/)
    }
    columns {
        status()
        weather()
